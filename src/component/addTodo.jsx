...
import { HiOutlineXCircle, HiPlusSmall } from "react-icons/hi2";
...
const AddTodo = (props) => {
  ...
  const onDelete = (itemOption) => {
    const resultDelCategory = props.category.filter(
      (c) => c.value === itemOption
    );
    props.setCategory(resultDelCategory);
  };
    ...
    <div>
      ...
      <div>
        ...
        <select
          name="categoreis2"
          id="categoreis2"
          value={props.categoryOption}
          onChange={(e) => props.setCategoryOption(e.target.value)}
        >
          {" "}
          <div>
            {" "}
          ...
          {props.category.map((op) => (
            <option key={op.value} value={op.value}>
              {op.lable}
              <div>
                <HiOutlineXCircle onClick={() => onDelete(op.value)} size={1} />
                sss
              </div>
            </option>
          ))}
